...
export function CollectorOpsPanel() {
  ...
  const previewLoopHits = mergeBatchMetric(
    ...
    Number(wechatAgentBatchStatus?.live_report_ocr_preview_seen_count || 0) + Number(wechatAgentBatchStatus?.live_report_ocr_title_seen_count || 0),
  );
    ...
    <section className="af-glass rounded-[30px] p-5 md:p-6">
      ...
      <div className="mt-4 rounded-2xl border border-white/80 bg-white/55 p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          ...
          />
          <StatCard
            label={localText(language, "daemonLastRun")}
            value={formatTs(daemonStatus?.last_run_at ?? null)}
          />
          <StatCard
            label={localText(language, "daemonSubmitMode")}
            value={daemonStatus?.last_run_submit_mode || "-"}
          />
          <StatCard
            label={localText(language, "daemonDiscovered")}
          ...
          />
          <StatCard
            label={localText(language, "daemonPluginCount")}
            value={daemonStatus?.last_run_plugin_count ?? 0}
          />
          <StatCard
            label={localText(language, "daemonUrlFallbackCount")}
            value={daemonStatus?.last_run_url_count ?? 0}
          />
          <StatCard
            label={localText(language, "daemonFailedCount")}
            ...
            {(daemonStatus?.last_rows || []).length ? (
              (daemonStatus?.last_rows || []).map((row, index) => (
                <div
                  ...
                  className="rounded-lg border border-slate-200/80 bg-slate-50/80 px-3 py-2 text-[11px] text-slate-700"
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-semibold text-slate-900">{row.article_token || "-"}</span>
                    <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] uppercase tracking-[0.08em] text-slate-700">
                      {row.mode || "-"}
                    </span>
                  ...
                  <p className="mt-1 text-slate-600">{row.note || "-"}</p>
                </div>
              ))
            ) : (
      ...
      <div className="mt-4 rounded-2xl border border-white/80 bg-white/55 p-4">
        ...
        {wechatAgentBatchStatus ? (
          <div className="mt-3 rounded-xl border border-white/80 bg-white/70 p-3 text-xs text-slate-600">
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-white/84 px-2.5 py-1 text-[11px] font-semibold">
                  {wechatAgentBatchStatus.route_quality.route_stability === "good"
                    ? "Route Quality · 健康"
                    : wechatAgentBatchStatus.route_quality.route_stability === "poor"
                      ? "Route Quality · 退化"
                ...
                <span className="rounded-full bg-white/84 px-2.5 py-1 text-[11px]">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">
              ...
              <StatCard label={localText(language, "wechatAgentBatchSubmittedOcr")} value={submittedOcr} />
              <StatCard label={localText(language, "wechatAgentBatchDedup")} value={wechatAgentBatchStatus.deduplicated_existing} />
              <StatCard label={localText(language, "wechatAgentBatchSeen")} value={skippedSeenTotal} />
              <StatCard label={localText(language, "wechatAgentBatchFailed")} value={failedTotal} />
            </div>
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">
              <StatCard label={localText(language, "wechatAgentBatchUrlFirstShare")} value={`${urlFirstShare}%`} />
              <StatCard label={localText(language, "wechatAgentBatchOcrShare")} value={`${ocrShare}%`} />
              <StatCard label={localText(language, "wechatAgentBatchDuplicateEscape")} value={duplicateEscapes} />
              <StatCard label={localText(language, "wechatAgentBatchHardEscape")} value={hardEscapes} />
              <StatCard label={localText(language, "wechatAgentBatchSubmenuTrap")} value={submenuTraps} />
              <StatCard label={localText(language, "wechatAgentBatchRouteBackoff")} value={routeBackoffs} />
              <StatCard label={localText(language, "wechatAgentBatchRouteBreaker")} value={routeCircuitBreakers} />
              <StatCard label={localText(language, "wechatAgentBatchRecoveries")} value={recoveryActions} />
            </div>
            <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
              <StatCard label={localText(language, "wechatAgentBatchUrlOnlySkips")} value={urlOnlySkips} />
              <StatCard label={localText(language, "wechatAgentBatchPreviewLoops")} value={previewLoopHits} />
              <StatCard label={localText(language, "wechatAgentBatchAccessibilityHits")} value={accessibilityHits} />
              <StatCard label={localText(language, "wechatAgentBatchTemplateHits")} value={templateHits} />
              <StatCard label={localText(language, "wechatAgentBatchPerceptualDedup")} value={perceptualDedupHits} />
            </div>
