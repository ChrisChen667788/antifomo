...
if [[ -z "$PYTHON_BIN" ]]; then
  if [[ -x "$ROOT_DIR/backend/.venv312/bin/python" ]]; then
    PYTHON_BIN="$ROOT_DIR/backend/.venv312/bin/python"
  elif [[ -x "$ROOT_DIR/backend/.venv/bin/python" ]]; then
    PYTHON_BIN="$ROOT_DIR/backend/.venv/bin/python"
  elif [[ -x "$ROOT_DIR/backend/.venv311/bin/python" ]]; then
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/wechat_pc_full_auto_agent.py" \
  --config "$CONFIG_FILE" \
...
if [[ -z "$INTERVAL_SEC" ]]; then
  INTERVAL_SEC="$("$PYTHON_BIN" - "$CONFIG_FILE" <<'PY'
import json
import sys
...
nohup bash -lc "
  cd '$ROOT_DIR'
  exec '$PYTHON_BIN' '$ROOT_DIR/scripts/wechat_pc_full_auto_agent.py' \
    --loop \
