...
def gather_items_in_window(
    ...
    *,
    ...
    # may exclude it. Clamp the lower bound to that second instead of widening by
    # a full extra second, so resumed sessions do not accidentally absorb paused-gap items.
    start_boundary = start_time.replace(microsecond=0)
    return list(
        db.scalars(
            select(Item)
...
def sync_session_items(db: Session, session: FocusSession, items: list[Item]) -> None:
    ...
    for item in items:
        if item.id in existing_ids:
            continue
        db.add(SessionItem(session=session, item=item))
        added = True
    if added:
        db.flush()
...
def sync_running_sessions_for_item(db: Session, item: Item) -> int:
    ...
    sessions = list(
        db.scalars(
            ...
            .where(
                or_(
                    FocusSession.current_window_started_at.is_(None),
                    FocusSession.current_window_started_at <= item.created_at,
                )
            )
        )
    )
...
def _resolved_current_window_start(session: FocusSession) -> datetime | None:
    return _ensure_utc(session.current_window_started_at) or _ensure_utc(session.start_time)
...
def calculate_elapsed_seconds(
    ...
    *,
    ...
    elapsed = max(int(session.elapsed_seconds or 0), 0)
    if session.status != "running":
        return elapsed

    current_start = _resolved_current_window_start(session)
    if current_start is None:
        return elapsed

    reference_now = _ensure_utc(now) or datetime.now(timezone.utc)
    delta = max(0, int((reference_now - current_start).total_seconds()))
...
def calculate_remaining_seconds(
    ...
    *,
    ...
    total_seconds = max(int(session.duration_minutes or 0) * 60, 0)
    return max(total_seconds - calculate_elapsed_seconds(session, now=now), 0)
...
def list_session_items(
    ...
    *,
    ...
    if session.session_items:
        persisted_items = [row.item for row in session.session_items if row.item is not None]
    else:
        persisted_items = list(
            db.scalars(
                select(Item)
                .join(SessionItem, SessionItem.item_id == Item.id)
                .where(SessionItem.session_id == session.id)
                .options(selectinload(Item.tags))
                .order_by(SessionItem.created_at.asc(), SessionItem.id.asc())
            )
        )
    if session.status != "running":
    ...
    live_items = gather_items_in_window(
        db,
        user_id=session.user_id,
        start_time=current_start,
        end_time=now or datetime.now(timezone.utc),
    )
    merged: dict[str, Item] = {str(item.id): item for item in persisted_items}
    ordered_items = list(persisted_items)
    for item in live_items:
        if str(item.id) in merged:
            continue
    ...
    *,
    ...
    if session.status == "running":
        ...
        if current_start is not None:
            current_items = gather_items_in_window(
                db,
                user_id=session.user_id,
                start_time=current_start,
                end_time=session.end_time,
            )
            sync_session_items(db, session, current_items)
            session.elapsed_seconds = calculate_elapsed_seconds(session, now=session.end_time)

    session.current_window_started_at = None
    session.paused_at = None
    session.status = "finished"

    items = list_session_items(db, session, now=session.end_time)
...
def pause_session(
    ...
    if current_start is not None:
        ...
        sync_session_items(db, session, current_items)
        session.elapsed_seconds = calculate_elapsed_seconds(session, now=paused_at)
    ...
    items = list_session_items(db, session, now=paused_at)
    metrics = compute_session_metrics(items)
    return session, items, metrics
...
def resume_session(
    ...
    if calculate_remaining_seconds(session) <= 0:
        raise ValueError("Session has no remaining time")

    resumed_at = datetime.now(timezone.utc)
    session.status = "running"
    session.paused_at = None
    session.current_window_started_at = resumed_at
    db.add(session)

    items = list_session_items(db, session, now=resumed_at)
    metrics = compute_session_metrics(items)
    return session, items, metrics
