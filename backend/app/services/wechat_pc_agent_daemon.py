...
class WechatAgentBatchStatus:
    ...
    duplicate_escape_count: int
    route_backoff_count: int
    route_circuit_breaker_count: int
    recovery_action_count: int
    url_only_skip_count: int
    ocr_preview_seen_count: int
    ocr_title_seen_count: int
    accessibility_action_hits: int
    template_match_hits: int
    perceptual_duplicate_count: int
    new_item_ids: list[str]
    last_message: str | None
    last_error: str | None
    hard_escape_count: int = 0
    submenu_trap_count: int = 0
    ...
    live_report_duplicate_escape_count: int = 0
    live_report_route_backoff_count: int = 0
    live_report_route_circuit_breaker_count: int = 0
    live_report_recovery_action_count: int = 0
    live_report_url_only_skip_count: int = 0
    live_report_ocr_preview_seen_count: int = 0
    live_report_ocr_title_seen_count: int = 0
    live_report_accessibility_action_hits: int = 0
    live_report_template_match_hits: int = 0
    live_report_perceptual_duplicate_count: int = 0
    live_report_hard_escape_count: int = 0
    live_report_submenu_trap_count: int = 0
...
def _default_batch_status_payload() -> dict[str, object]:
    return {
        ...
        "live_report_hard_escape_count": 0,
        "live_report_submenu_trap_count": 0,
...
def _sanitize_batch_status_payload(raw: dict[str, object]) -> dict[str, object]:
    ...
    payload["duplicate_escape_count"] = _coerce_int(raw.get("duplicate_escape_count"), 0, 0, 1_000_000)
    payload["route_backoff_count"] = _coerce_int(raw.get("route_backoff_count"), 0, 0, 1_000_000)
    payload["route_circuit_breaker_count"] = _coerce_int(raw.get("route_circuit_breaker_count"), 0, 0, 1_000_000)
    payload["recovery_action_count"] = _coerce_int(raw.get("recovery_action_count"), 0, 0, 1_000_000)
    payload["url_only_skip_count"] = _coerce_int(raw.get("url_only_skip_count"), 0, 0, 1_000_000)
    payload["ocr_preview_seen_count"] = _coerce_int(raw.get("ocr_preview_seen_count"), 0, 0, 1_000_000)
    payload["ocr_title_seen_count"] = _coerce_int(raw.get("ocr_title_seen_count"), 0, 0, 1_000_000)
    payload["accessibility_action_hits"] = _coerce_int(raw.get("accessibility_action_hits"), 0, 0, 1_000_000)
    payload["template_match_hits"] = _coerce_int(raw.get("template_match_hits"), 0, 0, 1_000_000)
    payload["perceptual_duplicate_count"] = _coerce_int(raw.get("perceptual_duplicate_count"), 0, 0, 1_000_000)
    payload["hard_escape_count"] = _coerce_int(raw.get("hard_escape_count"), 0, 0, 1_000_000)
    payload["submenu_trap_count"] = _coerce_int(raw.get("submenu_trap_count"), 0, 0, 1_000_000)
    ...
    payload["live_report_duplicate_escape_count"] = _coerce_int(
        raw.get("live_report_duplicate_escape_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_route_backoff_count"] = _coerce_int(
        raw.get("live_report_route_backoff_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_route_circuit_breaker_count"] = _coerce_int(
        raw.get("live_report_route_circuit_breaker_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_recovery_action_count"] = _coerce_int(
        raw.get("live_report_recovery_action_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_url_only_skip_count"] = _coerce_int(
        raw.get("live_report_url_only_skip_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_ocr_preview_seen_count"] = _coerce_int(
        raw.get("live_report_ocr_preview_seen_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_ocr_title_seen_count"] = _coerce_int(
        raw.get("live_report_ocr_title_seen_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_accessibility_action_hits"] = _coerce_int(
        raw.get("live_report_accessibility_action_hits"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_template_match_hits"] = _coerce_int(
        raw.get("live_report_template_match_hits"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_perceptual_duplicate_count"] = _coerce_int(
        raw.get("live_report_perceptual_duplicate_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_hard_escape_count"] = _coerce_int(
        raw.get("live_report_hard_escape_count"),
        0,
        0,
        1_000_000,
    )
    payload["live_report_submenu_trap_count"] = _coerce_int(
        raw.get("live_report_submenu_trap_count"),
        0,
        0,
        1_000_000,
    )
...
def _batch_status_from_payload(payload: dict[str, object]) -> WechatAgentBatchStatus:
    ...
    return WechatAgentBatchStatus(
        ...
        duplicate_escape_count=int(sanitized["duplicate_escape_count"]),
        route_backoff_count=int(sanitized["route_backoff_count"]),
        route_circuit_breaker_count=int(sanitized["route_circuit_breaker_count"]),
        recovery_action_count=int(sanitized["recovery_action_count"]),
        url_only_skip_count=int(sanitized["url_only_skip_count"]),
        ocr_preview_seen_count=int(sanitized["ocr_preview_seen_count"]),
        ocr_title_seen_count=int(sanitized["ocr_title_seen_count"]),
        accessibility_action_hits=int(sanitized["accessibility_action_hits"]),
        template_match_hits=int(sanitized["template_match_hits"]),
        perceptual_duplicate_count=int(sanitized["perceptual_duplicate_count"]),
        hard_escape_count=int(sanitized["hard_escape_count"]),
        submenu_trap_count=int(sanitized["submenu_trap_count"]),
        ...
        live_report_duplicate_escape_count=int(sanitized["live_report_duplicate_escape_count"]),
        live_report_route_backoff_count=int(sanitized["live_report_route_backoff_count"]),
        live_report_route_circuit_breaker_count=int(sanitized["live_report_route_circuit_breaker_count"]),
        live_report_recovery_action_count=int(sanitized["live_report_recovery_action_count"]),
        live_report_url_only_skip_count=int(sanitized["live_report_url_only_skip_count"]),
        live_report_ocr_preview_seen_count=int(sanitized["live_report_ocr_preview_seen_count"]),
        live_report_ocr_title_seen_count=int(sanitized["live_report_ocr_title_seen_count"]),
        live_report_accessibility_action_hits=int(sanitized["live_report_accessibility_action_hits"]),
        live_report_template_match_hits=int(sanitized["live_report_template_match_hits"]),
        live_report_perceptual_duplicate_count=int(sanitized["live_report_perceptual_duplicate_count"]),
        live_report_hard_escape_count=int(sanitized["live_report_hard_escape_count"]),
        live_report_submenu_trap_count=int(sanitized["live_report_submenu_trap_count"]),
...
def _sanitize_wechat_agent_config(raw: dict[str, object]) -> dict[str, object]:
    ...
    sanitized: dict[str, object] = {
        ...
        "rows_per_batch": _coerce_int(merged.get("rows_per_batch"), 2, 1, 20),
        "batches_per_cycle": _coerce_int(merged.get("batches_per_cycle"), 12, 1, 30),
        ...
        "scan_today_unread_only": bool(merged.get("scan_today_unread_only", True)),
...
def _read_live_report_snapshot() -> dict[str, object]:
    ...
    snapshot["live_report_duplicate_escape_count"] = _coerce_int(loaded.get("duplicate_escape_count"), 0, 0, 1_000_000)
    snapshot["live_report_route_backoff_count"] = _coerce_int(loaded.get("route_backoff_count"), 0, 0, 1_000_000)
    snapshot["live_report_route_circuit_breaker_count"] = _coerce_int(
        loaded.get("route_circuit_breaker_count"),
        0,
        0,
        1_000_000,
    )
    snapshot["live_report_recovery_action_count"] = _coerce_int(loaded.get("recovery_action_count"), 0, 0, 1_000_000)
    snapshot["live_report_url_only_skip_count"] = _coerce_int(loaded.get("url_only_skip_count"), 0, 0, 1_000_000)
    snapshot["live_report_ocr_preview_seen_count"] = _coerce_int(loaded.get("ocr_preview_seen_count"), 0, 0, 1_000_000)
    snapshot["live_report_ocr_title_seen_count"] = _coerce_int(loaded.get("ocr_title_seen_count"), 0, 0, 1_000_000)
    snapshot["live_report_accessibility_action_hits"] = _coerce_int(
        loaded.get("accessibility_action_hits"),
        0,
        0,
        1_000_000,
    )
    snapshot["live_report_template_match_hits"] = _coerce_int(loaded.get("template_match_hits"), 0, 0, 1_000_000)
    snapshot["live_report_perceptual_duplicate_count"] = _coerce_int(
        loaded.get("perceptual_duplicate_count"),
        0,
        0,
        1_000_000,
    )
    snapshot["live_report_hard_escape_count"] = _coerce_int(loaded.get("hard_escape_count"), 0, 0, 1_000_000)
    snapshot["live_report_submenu_trap_count"] = _coerce_int(loaded.get("submenu_trap_count"), 0, 0, 1_000_000)
    ...
    def worker() -> None:
        ...
        try:
            ...
            for segment_index in range(total_segments):
                ...
                payload["duplicate_escape_count"] = int(payload["duplicate_escape_count"]) + int(
                    _coerce_int(latest_report.get("duplicate_escape_count"), 0, 0, 1_000_000)
                )
                payload["route_backoff_count"] = int(payload["route_backoff_count"]) + int(
                    _coerce_int(latest_report.get("route_backoff_count"), 0, 0, 1_000_000)
                )
                payload["route_circuit_breaker_count"] = int(payload["route_circuit_breaker_count"]) + int(
                    _coerce_int(latest_report.get("route_circuit_breaker_count"), 0, 0, 1_000_000)
                )
                payload["recovery_action_count"] = int(payload["recovery_action_count"]) + int(
                    _coerce_int(latest_report.get("recovery_action_count"), 0, 0, 1_000_000)
                )
                payload["url_only_skip_count"] = int(payload["url_only_skip_count"]) + int(
                    _coerce_int(latest_report.get("url_only_skip_count"), 0, 0, 1_000_000)
                )
                payload["ocr_preview_seen_count"] = int(payload["ocr_preview_seen_count"]) + int(
                    _coerce_int(latest_report.get("ocr_preview_seen_count"), 0, 0, 1_000_000)
                )
                payload["ocr_title_seen_count"] = int(payload["ocr_title_seen_count"]) + int(
                    _coerce_int(latest_report.get("ocr_title_seen_count"), 0, 0, 1_000_000)
                )
                payload["accessibility_action_hits"] = int(payload["accessibility_action_hits"]) + int(
                    _coerce_int(latest_report.get("accessibility_action_hits"), 0, 0, 1_000_000)
                )
                payload["template_match_hits"] = int(payload["template_match_hits"]) + int(
                    _coerce_int(latest_report.get("template_match_hits"), 0, 0, 1_000_000)
                )
                payload["perceptual_duplicate_count"] = int(payload["perceptual_duplicate_count"]) + int(
                    _coerce_int(latest_report.get("perceptual_duplicate_count"), 0, 0, 1_000_000)
                )
                payload["hard_escape_count"] = int(payload["hard_escape_count"]) + int(
                    _coerce_int(latest_report.get("hard_escape_count"), 0, 0, 1_000_000)
                )
                payload["submenu_trap_count"] = int(payload["submenu_trap_count"]) + int(
                    _coerce_int(latest_report.get("submenu_trap_count"), 0, 0, 1_000_000)
                )
