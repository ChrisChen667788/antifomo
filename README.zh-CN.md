...
<table>
  ...
  <tr>
    ...
    <td width="50%">
      ...
      <p><strong>实验控制面</strong><br />固化 cohort、锁定 baseline、审计 rollout gate，并查看实际生效的运行时策略配置。</p>
    </td>
  </tr>
  <tr>
    <td width="50%">
...
- 可持久化实验编排层，支持策略计划配置、cohort 固化、版本 baseline 锁定、gate 历史、生效策略注册表、运行时策略快照、检索运行时配置和可审计 rollout manifest
