...
Current development version: `2.9.5+20260814`

Release promotion status: `blocked`. Real clarification/recovery tasks and feedback, human retrieval qrels, the 100-case primary review, 30-case second blind review, expert calibration, real three-industry blind evaluation, customer acceptance, production Skill governance, and final Office/visual/security evidence remain pending; machine-generated evidence is not treated as human approval.
...
| `1.7.2` | Evaluation scope feedback resolution | Applied 78 expert scope comments by replacing broad national/global regions and missing buyer/subject fields with concrete regions and named research targets. Preserved all reviewed answer anchors, source domains, and behavior labels; kept one unanswered but already concrete airport case unchanged. |
| `1.8.0` | Professional report semantics and formal artifacts | Added semantic delivery gates, claim/evidence ledgers, document-specific compilers, quantitative decision models, native DOCX/PPTX/PDF artifacts, proofreading, and machine Office/visual validation. |
| `1.8.1` | Release-readiness hardening | Aggregated health, diagnostics, low-quality review, independent review, and visual/Office evidence into one API/UI gate; added stability smoke and safe low-quality repair. |
| `1.8.2` | Evidence-bound research | Added scope contracts, source admission ledgers, evidence minimums, methodology routing, scope-isolated retrieval/archive reuse, and fail-closed evidence-gap output. |
| `1.8.3` | Question-tree and claim-level citation governance | Added six-axis question planning, bounded corrective retrieval, atomic claim/citation gates, hard score caps, and fail-closed solution generation; implementation is complete locally while independent release review remains blocked. |
| `1.8.4` | Expert calibration and unified hard failures | Added 100 primary plus 30 second blind review assignments, arbitration, five-dimension expert scoring, automatic-judge bias/recall calibration, and shared 20/40/59 score caps. |
| `1.9.0` | Architecture decision engineering | Added measurable QAW scenarios, ATAM findings, baseline/pilot/target ADRs, five C4 views, Well-Architected/NIST AI checks, 100% traceability, and zero-orphan-component validation. |
| `1.9.1` | Executable proof and acceptance evidence | Added machine-readable proof checks, a three-domain minimum vertical simulator, customer/internal evidence separation, digest-verified artifacts, and release-readiness integration; real human/customer evidence remains blocked. |
| `1.9.2` | Source-grounded semantic Notebook | Added source-scoped real SentenceTransformer retrieval, structured document adapters, immutable revisions, passage click-back, strict semantic blocking, and explicit lexical degradation. |
| `1.9.3` | China formal-document contracts | Added versioned government/enterprise feasibility, project-proposal, and solution policy packs with field states, material gaps, assumptions, and deterministic formula lineage. |
| `1.9.4` | Claim Graph and incremental section compiler | Added accepted-Claim DAGs, cycle/current-revision/trust validation, fact-conflict findings, bounded section compilation, and dependency-hash rebuild reuse. |
| `1.9.5` | Knowledge Space governance | Added owner/editor/reviewer/viewer ACL, source owner/expiry trust, review threads, connector fingerprints, allowlist dry-runs, and stale artifact backflow. |
| `1.9.6` | Governed Skills and controlled MCP | Added immutable Skill packages, HMAC signatures, licenses, quarantine, benchmarks, least-privilege dry-runs, built-in execution, and default-deny MCP invocation. |
| `2.0.0` | Evidence-bound Decision Studio | Added `/studio`, six evidence-bound artifact forms, shared Claim/source-revision lineage, cross-artifact consistency, and readiness that keeps all external commercial gates blocked until independently proven. |
| `2.0.1` - `2.0.6` | Immutable release program | Added real-data activation, 13 server-calculated acceptance suites, reviewer/artifact evidence contracts, append-only validation runs, SHA-256 audit chaining, performance/recovery probes, CLI automation, and a six-version release console while preserving all external blockers. |
| `2.0.7` - `2.2.0` | Decision Program engineering line | Added immutable RC evidence binding, editable/frozen research runs, hybrid retrieval and benchmark records, evidence-aware document blocks and Office confirmation, enterprise identity/ACL sync, governed durable Agents, three vertical packs, and inherited-readiness customer Pilots. External release acceptance remains blocked. |
| `2.2.1` | Evidence Recovery Contract | Added stable interaction states, bounded automatic recovery, policy-limited provisional reports, structured clarification packets, and hard formal-delivery guards. |
| `2.2.2` | Guided Clarification and Resume | Added typed answers, URL/text/file evidence, source-snapshot reuse, idempotent parent/child continuation, provenance, and bounded delta retrieval. |
| `2.2.3` | Progressive Disclosure Research UI | Added the result-first recovery card, plain-language status mapping, retained-progress messaging, and collapsed operator diagnostics. |
| `2.3.0` | Human-in-the-loop Research Experience RC | Added recovery conversion, cross-industry coverage, provenance, bypass, replay, and time-to-result telemetry with an explicit real-task readiness contract. |
| `2.3.1` | Clarification Quality and Operations | Added compact user feedback, stale-recovery and technical-copy monitoring, system-degraded retry, and global release-readiness integration. External acceptance remains blocked. |
| `2.3.2` | Entity and role truth | Added role-bearing organization evidence, locality-aware target pruning, tender-boilerplate hard negatives, and title/export protection against non-entities. |
| `2.3.3` | Evidence topology | Added pre-reranking source topology, local/current/primary/URL/snapshot checks, separate benchmark/context lanes, and no external-benchmark substitution for local opportunity proof. |
| `2.3.4` | Unified delivery truth | Added one formal/provisional/awaiting-user/system-degraded state, confidence caps for commercial sections, and fail-closed formal delivery. |
| `2.4.0` - `2.4.2` | Account pursuit and customer delivery | Added verified account cards, buyer/bid engineering, fact-assumption-benchmark-recommendation traceability, and a customer-specific architecture boundary. |
| `2.5.0` | Field-test calibration contract | Added 100-case quality-audit templates, per-case topology-qrel digests, paired fixed-evidence A/B evaluation, customer acceptance schema, and release-readiness aggregation. External calibration and acceptance remain blocked. |
| `2.5.1` - `2.6.5` | Research Assurance Command Center | Added a read-only 15-round quality program across report compatibility, source/entity/claim truth, delivery state, low-quality remediation, recovery, model/cost/reranker telemetry, cross-industry evidence, human artifacts, and visual/queue proof. The program is part of release readiness and preserves all missing external evidence as blocked. |
| `2.6.6` - `2.8.0` | Retrieval Assurance and Controlled Promotion | Added immutable retrieval result digests, fixed-cohort case diagnostics, complete-report paired review, bootstrap significance, actual Cross Encoder provenance, human approval separation, controlled shadow/drift evidence, baseline rollback, audit chaining, and a fail-closed release-readiness gate. |
| `2.8.1` - `2.9.5` | Retrieval Evidence Operations | Added evidence envelopes, eight-artifact inventory, exact lineage, freshness/chronology, named role separation, review coverage, incident and revocation records, canonical package digests, independent audit handoff, a Research Center operations panel, and a second fail-closed release gate. |
| `2.10.0` - `2.10.2` | Product-strategy evidence and HOLD-only artifact review | Added a first-party competitor-claim ledger, explicit `build`/`integrate`/`defer` decision context packets, and immutable delivery-artifact review drafts with field-level diffs. Office, visual, and attributable human acceptance evidence remain missing, so artifacts and release readiness stay blocked. |
