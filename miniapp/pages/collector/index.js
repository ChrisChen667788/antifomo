...
function buildBatchStatusView(status, i18n) {
  if (!hasBatchSnapshot(status)) {
    return {
      ...
      metrics: {
        ...
        accessibilityHits: 0,
        templateHits: 0,
        perceptualDedupHits: 0
      }
    };
  }
  const mergeMetric = (base, live) => (status.running ? Number(base || 0) + Number(live || 0) : Number(base || 0));
  ...
  const progress = totalSegments
    ? status.running
      ...
      : status.finished_at
        ...
        : 0
    : 0;
  const submittedUrl = mergeMetric(status.submitted_url, status.live_report_submitted_url);
  const submittedUrlDirect = mergeMetric(status.submitted_url_direct, status.live_report_submitted_url_direct);
  const submittedUrlShareCopy = mergeMetric(status.submitted_url_share_copy, status.live_report_submitted_url_share_copy);
  const submittedUrlResolved = mergeMetric(status.submitted_url_resolved, status.live_report_submitted_url_resolved);
  const submittedOcr = mergeMetric(status.submitted_ocr, status.live_report_submitted_ocr);
  const submittedTotal = mergeMetric(status.submitted, status.live_report_submitted);
  const duplicateEscapes = mergeMetric(status.duplicate_escape_count, status.live_report_duplicate_escape_count);
  const routeBackoffs = mergeMetric(status.route_backoff_count, status.live_report_route_backoff_count);
  const routeBreakers = mergeMetric(status.route_circuit_breaker_count, status.live_report_route_circuit_breaker_count);
  const recoveryActions = mergeMetric(status.recovery_action_count, status.live_report_recovery_action_count);
  const urlOnlySkips = mergeMetric(status.url_only_skip_count, status.live_report_url_only_skip_count);
  const skippedSeen = mergeMetric(status.skipped_seen, status.live_report_skipped_seen);
  const failed = mergeMetric(status.failed, status.live_report_failed);
  const previewLoopHits = mergeMetric(
    Number(status.ocr_preview_seen_count || 0) + Number(status.ocr_title_seen_count || 0),
    Number(status.live_report_ocr_preview_seen_count || 0) + Number(status.live_report_ocr_title_seen_count || 0)
  );
  const accessibilityHits = mergeMetric(status.accessibility_action_hits, status.live_report_accessibility_action_hits);
  const templateHits = mergeMetric(status.template_match_hits, status.live_report_template_match_hits);
  const perceptualDedupHits = mergeMetric(status.perceptual_duplicate_count, status.live_report_perceptual_duplicate_count);
  ...
  return {
    ...
    summary: status.running
      ? `第 ${Math.max(currentSegment, 1)}/${Math.max(totalSegments, 1)} 段 · ${i18n.wechatAgentBatchSubmitted} ${submittedTotal} ${i18n.itemsUnit}`
      : `第 ${Math.max(totalSegments, 0)} ${i18n.wechatAgentBatchSegments} · ${i18n.wechatAgentBatchSubmitted} ${submittedTotal} ${i18n.itemsUnit}`,
...
Page({
  ...
  applyPreferences() {
    ...
    const i18n = {
      ...
      wechatAgentBatchUrlFirstShare: textByLanguage(language, {
        "zh-CN": "URL-first 占比",
        "zh-TW": "URL-first 佔比",
        en: "URL-first share",
        ja: "URL-first 比率",
      ...
      wechatAgentBatchOcrShare: textByLanguage(language, {
        "zh-CN": "OCR 占比",
        "zh-TW": "OCR 佔比",
        en: "OCR share",
        ja: "OCR 比率",
      ...
      wechatAgentBatchDuplicateEscape: textByLanguage(language, {
        "zh-CN": "重复脱困",
        "zh-TW": "重複脫困",
      ...
      wechatAgentBatchRouteBackoff: textByLanguage(language, {
        "zh-CN": "Route 回退",
        "zh-TW": "Route 回退",
        en: "Route backoff",
        ja: "Route バックオフ",
      ...
      wechatAgentBatchRouteBreaker: textByLanguage(language, {
        "zh-CN": "Route 熔断",
        "zh-TW": "Route 熔斷",
        en: "Route breaker",
        ja: "Route ブレーカー",
      ...
      wechatAgentBatchRecoveries: textByLanguage(language, {
        "zh-CN": "恢复动作",
        "zh-TW": "恢復動作",
        en: "Recoveries",
        ja: "復帰アクション",
      ...
      wechatAgentBatchUrlOnlySkips: textByLanguage(language, {
        "zh-CN": "URL-only 跳过",
        "zh-TW": "URL-only 跳過",
        en: "URL-only skips",
        ja: "URL-only スキップ",
      ...
      wechatAgentBatchPreviewLoops: textByLanguage(language, {
        "zh-CN": "同文命中",
        "zh-TW": "同文命中",
      ...
      wechatAgentBatchAccessibilityHits: textByLanguage(language, {
        "zh-CN": "AX 命中",
        "zh-TW": "AX 命中",
        en: "AX hits",
        ja: "AX ヒット",
      ...
      wechatAgentBatchTemplateHits: textByLanguage(language, {
        "zh-CN": "模板命中",
        "zh-TW": "模板命中",
        en: "Template hits",
        ja: "テンプレートヒット",
      ...
      wechatAgentBatchPerceptualDedup: textByLanguage(language, {
        "zh-CN": "感知去重",
        "zh-TW": "感知去重",
        en: "Perceptual dedup",
        ja: "知覚重複除外",
