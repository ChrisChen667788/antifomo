...
class ResearchTrackingTopic(Base):
    ...
    compare_snapshots: Mapped[list["ResearchCompareSnapshot"]] = relationship(
        back_populates="tracking_topic",
        foreign_keys="ResearchCompareSnapshot.tracking_topic_id",
    )
    markdown_archives: Mapped[list["ResearchMarkdownArchive"]] = relationship(
        back_populates="tracking_topic",
        foreign_keys="ResearchMarkdownArchive.tracking_topic_id",
    )
...
class ResearchReportVersion(Base):
    ...
    compare_snapshots: Mapped[list["ResearchCompareSnapshot"]] = relationship(
        ...
        foreign_keys="ResearchCompareSnapshot.report_version_id",
    )
    markdown_archives: Mapped[list["ResearchMarkdownArchive"]] = relationship(
        back_populates="report_version",
        foreign_keys="ResearchMarkdownArchive.report_version_id",
    )


class ResearchCompareSnapshot(Base):
    __tablename__ = "research_compare_snapshots"
    __table_args__ = (
        Index("idx_research_compare_snapshots_user_updated_at", "user_id", "updated_at"),
        Index("idx_research_compare_snapshots_topic_id", "tracking_topic_id"),
        Index("idx_research_compare_snapshots_report_version_id", "report_version_id"),
    )
    ...
    tracking_topic: Mapped[Optional["ResearchTrackingTopic"]] = relationship(
        back_populates="compare_snapshots",
        foreign_keys=[tracking_topic_id],
    )
    report_version: Mapped[Optional["ResearchReportVersion"]] = relationship(
        back_populates="compare_snapshots",
        foreign_keys=[report_version_id],
    )
    markdown_archives: Mapped[list["ResearchMarkdownArchive"]] = relationship(
        back_populates="compare_snapshot",
        foreign_keys="ResearchMarkdownArchive.compare_snapshot_id",
    )


class ResearchMarkdownArchive(Base):
    __tablename__ = "research_markdown_archives"
    __table_args__ = (
        Index("idx_research_markdown_archives_user_updated_at", "user_id", "updated_at"),
        Index("idx_research_markdown_archives_topic_id", "tracking_topic_id"),
        Index("idx_research_markdown_archives_snapshot_id", "compare_snapshot_id"),
        Index("idx_research_markdown_archives_report_version_id", "report_version_id"),
    )
    ...
    compare_snapshot_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("research_compare_snapshots.id", ondelete="SET NULL"),
        nullable=True,
    )
    report_version_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        Uuid(as_uuid=True),
    ...
    archive_kind: Mapped[str] = mapped_column(String(40), nullable=False, default="compare_markdown", server_default="compare_markdown")
    ...
    tracking_topic: Mapped[Optional["ResearchTrackingTopic"]] = relationship(
        back_populates="markdown_archives",
        foreign_keys=[tracking_topic_id],
    )
    compare_snapshot: Mapped[Optional["ResearchCompareSnapshot"]] = relationship(
        back_populates="markdown_archives",
        foreign_keys=[compare_snapshot_id],
    )
    report_version: Mapped[Optional["ResearchReportVersion"]] = relationship(
        back_populates="markdown_archives",
        foreign_keys=[report_version_id],
...
class ResearchExperimentPlan(Base):
    ...
    gate_history_payload: Mapped[list[dict]] = mapped_column(JSON, nullable=False, default=list)
    rollout_payload: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    ...
    rollout_revoked_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
