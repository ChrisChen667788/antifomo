...
def test_research_conversation_bootstrap_and_followup() -> None:
    ...
    try:
        ...
        report_payload = {
            ...
            "followup_context": {
                "supplemental_evidence": "新增证据显示，上海数据集团 7 月前后将启动新一轮预算复核。",
            },
            "sections": [
                {
                    "title": "项目与商机判断",
                    "items": ["上海数据集团更适合优先推进，预算和时间窗都更清晰。"],
                    "evidence_links": [
                        {
                            "title": "上海数据集团公开公告",
                            ...
                            "excerpt": "公告提到 7 月前后将启动预算复核与下一轮需求确认。",
                        }
                    ],
                }
            ],
        ...
        assert updated["messages"][-1]["payload"]["retrieval_matches"]
        assert any(
            "预算复核" in str(match.get("text") or "")
            for match in updated["messages"][-1]["payload"]["retrieval_matches"]
        )
    finally:
        db.close()
