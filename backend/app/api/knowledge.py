...
from app.schemas.knowledge import (
    KnowledgeAccountDetailOut,
    KnowledgeAccountListResponse,
    KnowledgeCommercialDashboardOut,
    ...
    KnowledgeReviewQueueResolutionRequest,
)
...
def _review_queue_item_exists(payload: dict | None, review_id: str) -> bool:
    ...
    return any(isinstance(item, dict) and str(item.get("id") or "").strip() == normalized_id for item in raw_queue)
...
@router.get("/dashboard", response_model=KnowledgeCommercialDashboardOut)
def get_knowledge_dashboard(db: Session = Depends(get_db)) -> KnowledgeCommercialDashboardOut:
    ensure_demo_user(db)
    return KnowledgeCommercialDashboardOut(**build_knowledge_commercial_dashboard(db))


@router.get("/accounts", response_model=KnowledgeAccountListResponse)
def get_knowledge_accounts(
    limit: int = 20,
    query: str | None = None,
    db: Session = Depends(get_db),
) -> KnowledgeAccountListResponse:
    ensure_demo_user(db)
    return KnowledgeAccountListResponse(items=list_knowledge_accounts(db, query=query, limit=limit))


@router.get("/accounts/{account_slug}", response_model=KnowledgeAccountDetailOut)
def get_knowledge_account(
    account_slug: str,
    db: Session = Depends(get_db),
) -> KnowledgeAccountDetailOut:
    ensure_demo_user(db)
    detail = get_knowledge_account_detail(db, account_slug)
    if detail is None:
        raise HTTPException(status_code=404, detail="Knowledge account not found")
    return KnowledgeAccountDetailOut(**detail)


@router.get("/opportunities", response_model=KnowledgeOpportunityListResponse)
def get_knowledge_opportunities(
    limit: int = 30,
    account_slug: str | None = None,
    db: Session = Depends(get_db),
) -> KnowledgeOpportunityListResponse:
    ensure_demo_user(db)
    return KnowledgeOpportunityListResponse(
        items=list_knowledge_opportunities(db, account_slug=account_slug, limit=limit)
    )


@router.post("/backfill/research-intelligence")
def run_knowledge_research_backfill(db: Session = Depends(get_db)) -> dict[str, int]:
    ensure_demo_user(db)
    return backfill_research_knowledge_intelligence(db)
...
@router.post("/{entry_id}/review-queue/{review_id}", response_model=KnowledgeEntryOut)
def resolve_knowledge_review_queue_item(
    entry_id: UUID,
    review_id: str,
    payload: KnowledgeReviewQueueResolutionRequest,
    db: Session = Depends(get_db),
) -> KnowledgeEntryOut:
    ensure_demo_user(db)
    entry = _get_entry_or_404(db, entry_id)
    metadata_payload = entry.metadata_payload if isinstance(entry.metadata_payload, dict) else {}
    resolved_payload = apply_review_queue_resolutions(metadata_payload)
    if not _review_queue_item_exists(resolved_payload, review_id):
        raise HTTPException(status_code=404, detail="Review queue item not found")
    entry.metadata_payload = update_review_queue_resolution(
        resolved_payload,
        review_id=review_id,
