...
def wait_items_until_minimum_terminal(
    api_base: str,
    item_ids: list[str],
    *,
    minimum_terminal: int = 1,
    ...
    terminal_count = 0
    target_terminal = min(len(item_ids), max(1, minimum_terminal))
    while pending and time.time() < deadline:
        for item_id in list(pending):
            ...
            if status in {"ready", "failed"}:
                ...
                terminal_count += 1
        if terminal_count >= target_terminal:
            break
        if pending:
...
def ensure_ready_items_for_urls(api_base: str, urls: list[str], minimum_ready: int = 5) -> list[dict[str, Any]]:
    ready_items = find_latest_ready_items_for_urls(api_base, urls, limit=max(500, len(urls) * 8))
    target_ready = min(len(urls), max(1, minimum_ready))
    ready_by_url = {str(item.get("source_url") or ""): item for item in ready_items if item.get("source_url")}
    if len(ready_by_url) >= target_ready:
        return ready_items
    missing_urls = [url for url in urls if url not in ready_by_url]
    if not missing_urls:
        return ready_items
    ...
    batch = api_call(
        ...
        "/api/items/batch",
        method="POST",
        payload={"source_type": "url", "urls": missing_urls, "deduplicate": False},
    )
    created_ids = [row["item_id"] for row in batch.get("results", []) if row.get("status") == "created" and row.get("item_id")]
    if created_ids:
        ...
        while pending and time.time() < deadline:
            ...
            if pending:
                time.sleep(2.0)
    return find_latest_ready_items_for_urls(api_base, urls, limit=max(500, len(urls) * 8))
...
def run_simulation(
    ...
    ready_items = ensure_ready_items_for_urls(api_base, urls)
    ...
    if created_ids:
        wait_items_until_minimum_terminal(
            api_base,
            created_ids,
            minimum_terminal=min(2, len(created_ids)),
            timeout_seconds=90,
            poll_interval=2.0,
        )
