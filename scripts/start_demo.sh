...
start_frontend() {
  ...
  local frontend_pid
  frontend_pid="$(
    ROOT_DIR="$ROOT_DIR" BACKEND_URL="$BACKEND_URL" FRONTEND_LOG="$FRONTEND_LOG" FRONTEND_NODE="$FRONTEND_NODE" FRONTEND_PORT="$FRONTEND_PORT" python3 - <<'PY'
import os
import subprocess
...
proc = subprocess.Popen(
    [
        ...
        (
            ...
            f"exec '{frontend_node}' '{os.environ['ROOT_DIR']}/node_modules/next/dist/bin/next' dev --webpack --port {os.environ['FRONTEND_PORT']}"
        ),
    ],
    stdin=subprocess.DEVNULL,
    stdout=log,
