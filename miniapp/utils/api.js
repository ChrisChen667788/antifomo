        ...
        ? entity.evidence_links
            .map((link) => {
              ...
              return {
                ...
                anchor_text: (link && link.anchor_text) || ""
              };
            })
  ...
  return {
    ...
    sections: Array.isArray(normalized.sections)
      ? normalized.sections.map((section) => ({
          ...
          source_tier_counts:
            section && section.source_tier_counts && typeof section.source_tier_counts === "object"
              ? section.source_tier_counts
              : {},
          official_source_ratio: Number((section && section.official_source_ratio) || 0),
          evidence_links: Array.isArray(section && section.evidence_links)
      ...
      normalized && normalized.source_diagnostics && typeof normalized.source_diagnostics === "object"
        ? {
            ...
            strategy_exclusion_terms: Array.isArray(normalized.source_diagnostics.strategy_exclusion_terms)
              ...
              : [],
            pipeline_summary:
              typeof normalized.source_diagnostics.pipeline_summary === "string"
                ? normalized.source_diagnostics.pipeline_summary
                : "",
            pipeline_stages: Array.isArray(normalized.source_diagnostics.pipeline_stages)
              ? normalized.source_diagnostics.pipeline_stages
                  .map((stage) => ({
...
function getResearchWatchlistAutomationStatus() {
  return request("/api/research/watchlists/automation-status", {
    method: "GET"
  }).catch(() => ({
    installed: false,
    loaded: false,
...
function updateResearchWatchlist(watchlistId, payload) {
  return request(`/api/research/watchlists/${watchlistId}`, {
    method: "PATCH",
    data: payload || {}
  }).catch(() => ({
    id: watchlistId,
    tracking_topic_id: null,
    name: (payload && payload.name) || "本地 Watchlist",
    watch_type: "topic",
    query: (payload && payload.query) || "",
    research_focus: (payload && payload.research_focus) || "",
    perspective: (payload && payload.perspective) || "all",
...
function runDueResearchWatchlists(payload) {
  const limit = payload && typeof payload.limit === "number" ? `?limit=${encodeURIComponent(String(payload.limit))}` : "";
  const bodyPayload = { ...(payload || {}) };
  if (Object.prototype.hasOwnProperty.call(bodyPayload, "limit")) {
    delete bodyPayload.limit;
  }
  return request(`/api/research/watchlists/run-due${limit}`, {
    method: "POST",
    data: bodyPayload
  }).catch(() => ({
...
function getCollectorDaemonStatus() {
  return request("/api/collector/daemon/status")
    ...
    .catch(() => ({
      ...
      log_size_bytes: 0,
      source_file_count: 0,
      last_run_handled_count: 0,
      last_run_coverage_rate: 0,
      last_run_body_success_rate: 0,
      coverage_state: "idle",
