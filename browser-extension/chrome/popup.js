...
const manualBatchInfoEl = document.getElementById("manual-batch-info");
...
const batchSendManualBtn = document.getElementById("batch-send-manual-btn");
...
function setBatchInfo(message) {
  batchInfoEl.textContent = message;
}

function setManualBatchInfo(message) {
  manualBatchInfoEl.textContent = message;
}
...
function normalizeArticleUrl(value) {
  ...
  try {
    ...
    const isWechatArticle =
      /mp\.weixin\.qq\.com\/s(\/|\?)/i.test(href) || /mp\.weixin\.qq\.com\/mp\/appmsg/i.test(href);
    return isWechatArticle ? href : "";
  } catch {
    return "";
  }
}

function parseManualBatchLinks(value) {
  const tokens = String(value || "").match(/https?:\/\/[^\s,，]+/gi) || [];
  const seen = new Set();
  const output = [];
  for (const token of tokens) {
    const normalized = normalizeArticleUrl(token);
...
function refreshManualBatchState() {
  const links = parseManualBatchLinks(batchLinksInput?.value || "");
  if (!links.length) {
    setManualBatchInfo("粘贴公众号文章链接后可直接批量入库。");
    batchSendManualBtn.disabled = true;
    return links;
  }
  setManualBatchInfo(`已识别 ${links.length} 条有效公众号文章链接，可直接批量发送。`);
  batchSendManualBtn.disabled = false;
  return links;
}
    ...
    const results = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        const isWeChatArticleUrl = (value) =>
          /mp\.weixin\.qq\.com\/s(\/|\?)/i.test(value) || /mp\.weixin\.qq\.com\/mp\/appmsg/i.test(value);
        const pickMeta = (...keys) => {
        ...
        const pushLink = (value) => {
          const next = sanitizeUrl(value);
          if (!next || seenLinks.has(next) || !next.includes("mp.weixin.qq.com") || !isWeChatArticleUrl(next)) {
            return;
          }
          seenLinks.add(next);
          discoveredLinks.push(next);
        };
        if (isWeChat) {
        ...
        return {
          ...
          is_wechat: isWeChat,
          discovered_links: discoveredLinks.slice(0, 50),
        };
      },
    });
...
async function sendBatchLinks(sourceUrls, button, statusPrefix) {
  ...
  if (!Array.isArray(sourceUrls) || !sourceUrls.length) {
    setStatus(`${statusPrefix}未发现可发送的公众号文章链接。`, "error");
    return;
  }

  button.disabled = true;
  setStatus(`${statusPrefix}发送中... (${sourceUrls.length})`);
  try {
    const response = await fetch(`${apiBase}/api/collector/browser/batch-ingest`, {
      method: "POST",
    ...
    setStatus(
      `${statusPrefix}完成：created=${result?.created || 0}, deduplicated=${result?.deduplicated || 0}, failed=${result?.failed || 0}`,
      "ok",
    );
  } catch (error) {
    setStatus(`${statusPrefix}失败：${error?.message || "未知错误"}`, "error");
  } finally {
    if (button === batchSendManualBtn) {
      refreshManualBatchState();
    } else {
...
async function sendBatchFromCurrentPage() {
  if (!discoveredBatchLinks.length) {
    setStatus("当前页未发现可批量发送的公众号文章链接。", "error");
    return;
  }
  await sendBatchLinks(discoveredBatchLinks, batchSendBtn, "当前页批量发送");
}

async function sendBatchFromManualInput() {
  const links = refreshManualBatchState();
  if (!links.length) {
    setStatus("请先粘贴有效的公众号文章链接。", "error");
    return;
  }
  await sendBatchLinks(links, batchSendManualBtn, "手动批量发送");
}
...
async function init() {
  ...
  discoveredBatchLinks = Array.isArray(extractedPage?.discovered_links)
    ? extractedPage.discovered_links.filter(Boolean)
    : [];
  if (extractedPage?.title) {
  ...
  if (discoveredBatchLinks.length > 1) {
    setBatchInfo(`发现 ${discoveredBatchLinks.length} 条公众号文章链接，可直接批量入库。`);
    batchSendBtn.disabled = false;
  } else if (discoveredBatchLinks.length === 1) {
    setBatchInfo("当前页是单篇公众号文章，建议直接用上方单页发送。");
    batchSendBtn.disabled = true;
  } else {
    setBatchInfo("当前页未发现可批量发送的公众号文章链接。");
    batchSendBtn.disabled = true;
  }
...
batchSendBtn.addEventListener("click", () => {
  void sendBatchFromCurrentPage();
});

batchSendManualBtn.addEventListener("click", () => {
  void sendBatchFromManualInput();
});

batchLinksInput.addEventListener("input", () => {
  refreshManualBatchState();
});
