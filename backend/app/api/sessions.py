...
def _build_session_out(session: FocusSession, items: list[Item]) -> SessionOut:
    ...
    return SessionOut(
        ...
        current_window_started_at=_ensure_utc(session.current_window_started_at),
        paused_at=_ensure_utc(session.paused_at),
        elapsed_seconds=calculate_elapsed_seconds(session, now=now),
        remaining_seconds=calculate_remaining_seconds(session, now=now),
...
def start_session(payload: SessionStartRequest, db: Session = Depends(get_db)) -> SessionOut:
    ...
    session = FocusSession(
        ...
        current_window_started_at=started_at,
        status="running",
    )
    db.add(session)
    db.commit()
...
def finish_user_session(
    ...
    try:
        session, items, _ = finish_session(db, session, output_language=resolved_language)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

    db.commit()
    db.refresh(session)
    return SessionFinishResponse(session=_build_session_out(session, items))


@router.post("/{session_id}/pause", response_model=SessionPauseResponse)
def pause_user_session(session_id: UUID, db: Session = Depends(get_db)) -> SessionPauseResponse:
    ensure_demo_user(db)
    session = _get_session_or_404(db, session_id)

    try:
        session, items, _ = pause_session(db, session)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

    db.commit()
    db.refresh(session)
    return SessionPauseResponse(session=_build_session_out(session, items))


@router.post("/{session_id}/resume", response_model=SessionResumeResponse)
def resume_user_session(session_id: UUID, db: Session = Depends(get_db)) -> SessionResumeResponse:
    ensure_demo_user(db)
    session = _get_session_or_404(db, session_id)

    try:
        session, items, _ = resume_session(db, session)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

    db.commit()
    db.refresh(session)
    return SessionResumeResponse(session=_build_session_out(session, items))
