...
function buildWatchlistScheduleOptions(i18n) {
  return [
    { value: "manual", label: (i18n && i18n.watchlistScheduleManual) || "手动" },
    { value: "daily", label: (i18n && i18n.watchlistScheduleDaily) || "每日" },
    { value: "twice_daily", label: (i18n && i18n.watchlistScheduleTwiceDaily) || "每日两次" },
    { value: "weekdays", label: (i18n && i18n.watchlistScheduleWeekdays) || "工作日" },
    { value: "every_6h", label: (i18n && i18n.watchlistScheduleEvery6h) || "每 6 小时" },
  ];
}

function normalizeWatchlist(watchlist, scheduleOptions) {
  const options = Array.isArray(scheduleOptions) ? scheduleOptions : [];
  const schedule = String((watchlist && watchlist.schedule) || "manual");
  const scheduleIndex = Math.max(0, options.findIndex((item) => item.value === schedule));
  return {
    ...watchlist,
    schedule,
    schedule_index: scheduleIndex,
    schedule_label: (options[scheduleIndex] && options[scheduleIndex].label) || schedule,
    last_checked_label: watchlist && watchlist.last_checked_at ? formatDateLabel(watchlist.last_checked_at) : "",
    next_due_label: watchlist && watchlist.next_due_at ? formatDateLabel(watchlist.next_due_at) : "",
    is_due: !!(watchlist && watchlist.is_due),
    latest_changes: Array.isArray(watchlist && watchlist.latest_changes) ? watchlist.latest_changes.slice(0, 3) : [],
  };
}

function normalizeWatchlistAutomationStatus(status) {
  if (!status) return null;
  return {
    ...status,
    interval_label: formatIntervalLabel(status.interval_seconds),
    last_checked_label: status.last_checked_at ? formatDateLabel(status.last_checked_at) : "",
  };
}
...
Page({
  data: {
    ...
    trackingTopics: [],
    watchlists: [],
    watchlistAutomationStatus: null,
  ...
  applyPreferences() {
    ...
    this.setData({
      ...
      i18n: {
        ...
        watchlistAutomationLoaded: t(language, "research.watchlistAutomationLoaded", "本地自动巡检已加载"),
        watchlistAutomationInstalled: t(language, "research.watchlistAutomationInstalled", "已安装，等待 launchd 加载"),
        watchlistAutomationMissing: t(language, "research.watchlistAutomationMissing", "尚未安装本地自动巡检"),
        watchlistAutomationInterval: t(language, "research.watchlistAutomationInterval", "巡检间隔"),
        watchlistAutomationLastRun: t(language, "research.watchlistAutomationLastRun", "最近自动运行"),
        watchlistAutomationHint: t(language, "research.watchlistAutomationHint", "建议通过 launchd 每小时触发一次本地 watchlist 脚本，只刷新已到期 watchlist。"),
        watchlistAutomationInstallCmd: t(language, "research.watchlistAutomationInstallCmd", "安装命令"),
        watchlistAutomationUninstallCmd: t(language, "research.watchlistAutomationUninstallCmd", "卸载命令"),
        watchlistAutomationDue: t(language, "research.watchlistAutomationDue", "最近到期"),
        watchlistAutomationRefreshed: t(language, "research.watchlistAutomationRefreshed", "最近刷新"),
        watchlistAutomationFailed: t(language, "research.watchlistAutomationFailed", "最近失败"),
        ...
        runDueWatchlists: t(language, "research.watchlistRunDue", "执行到期 Watchlist"),
        runningDueWatchlists: t(language, "research.watchlistRunDueRunning", "执行中..."),
        watchlistDueNow: t(language, "research.watchlistDueNow", "已到刷新窗口"),
        watchlistLastChecked: t(language, "research.watchlistLastChecked", "最近检查"),
        watchlistNextDue: t(language, "research.watchlistNextDue", "下次到期"),
        watchlistScheduleManual: t(language, "research.watchlistSchedule.manual", "手动"),
        watchlistScheduleDaily: t(language, "research.watchlistSchedule.daily", "每日"),
        watchlistScheduleTwiceDaily: t(language, "research.watchlistSchedule.twice_daily", "每日两次"),
        watchlistScheduleWeekdays: t(language, "research.watchlistSchedule.weekdays", "工作日"),
        watchlistScheduleEvery6h: t(language, "research.watchlistSchedule.every_6h", "每 6 小时"),
      ...
      watchlistScheduleOptions: buildWatchlistScheduleOptions({
        watchlistScheduleManual: t(language, "research.watchlistSchedule.manual", "手动"),
        watchlistScheduleDaily: t(language, "research.watchlistSchedule.daily", "每日"),
        watchlistScheduleTwiceDaily: t(language, "research.watchlistSchedule.twice_daily", "每日两次"),
        watchlistScheduleWeekdays: t(language, "research.watchlistSchedule.weekdays", "工作日"),
      ...
      if (Array.isArray(this.data.watchlists) && this.data.watchlists.length) {
        this.setData({
          watchlists: this.data.watchlists.map((item) => normalizeWatchlist(item, this.data.watchlistScheduleOptions)),
        });
      }
    });
  },
  ...
  loadWorkspace() {
    Promise.all([getResearchWorkspace(), listResearchWatchlists(), getResearchWatchlistAutomationStatus()])
      .then(([res, watchlists, automationStatus]) => {
        ...
        this.setData({
          ...
          trackingTopics,
          watchlists: Array.isArray(watchlists)
            ? watchlists.map((item) => normalizeWatchlist(item, this.data.watchlistScheduleOptions))
            : [],
          watchlistAutomationStatus: normalizeWatchlistAutomationStatus(automationStatus),
  ...
  createWatchlistFromTopic(e) {
    ...
    createResearchWatchlist({
      ...
      .then((saved) => {
        const normalized = normalizeWatchlist(saved, this.data.watchlistScheduleOptions);
        const next = [normalized].concat((this.data.watchlists || []).filter((item) => item.id !== saved.id));
  ...
  refreshWatchlist(e) {
    ...
    refreshResearchWatchlist(id, {
      ...
      .then((res) => {
        const nextWatchlists = (this.data.watchlists || []).map((item) =>
          (item.id === id ? normalizeWatchlist(res.watchlist, this.data.watchlistScheduleOptions) : item)
        );
  ...
  onWatchlistScheduleChange(e) {
    const id = e.currentTarget.dataset.id;
    const index = Number((e.detail && e.detail.value) || 0);
    const option = (this.data.watchlistScheduleOptions || [])[index];
    if (!id || !option) return;
    updateResearchWatchlist(id, { schedule: option.value })
      .then((saved) => {
        const next = (this.data.watchlists || []).map((item) =>
          item.id === id ? normalizeWatchlist(saved, this.data.watchlistScheduleOptions) : item
        );
        this.setData({ watchlists: next });
      })
      .catch(() => {
        wx.showToast({
          title: "更新失败",
  ...
  runDueWatchlists() {
    if (this.data.runningDueWatchlists) return;
    this.setData({ runningDueWatchlists: true });
    runDueResearchWatchlists({
      output_language: "zh-CN",
      include_wechat: true,
      max_sources: 12,
      ...
      .then(() => Promise.all([getResearchWorkspace(), listResearchWatchlists()]))
      .then(([workspace, watchlists]) => {
        this.setData({
          trackingTopics: Array.isArray(workspace.tracking_topics) ? workspace.tracking_topics.map(normalizeTrackingTopic) : [],
          watchlists: Array.isArray(watchlists)
            ? watchlists.map((item) => normalizeWatchlist(item, this.data.watchlistScheduleOptions))
            : [],
        });
      ...
      .finally(() => {
        this.setData({ runningDueWatchlists: false });
      });
  },
