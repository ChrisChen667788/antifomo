...
type RankedGroupTone = "sky" | "amber" | "emerald";
...
function rankedGroupTone(tone: RankedGroupTone) {
  if (tone === "amber") {
    return {
      panelClass:
                  ...
                  {(meta.topTargets.length || meta.topCompetitors.length || meta.topPartners.length) ? (
                    <div className="mt-4 grid gap-3 md:grid-cols-3">
                        ...
                        <div className={`rounded-[22px] border p-3.5 ${rankedGroupTone("sky").panelClass}`}>
                          ...
                          <div className="mt-2 flex flex-wrap gap-2">
                            {meta.topTargets.map((value) => (
                              <span
                                key={value}
                                className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs ${rankedGroupTone("sky").pillClass}`}
                              >
                                <span className={`h-1.5 w-1.5 rounded-full ${rankedGroupTone("sky").dotClass}`} />
                                {value}
                              </span>
                            ))}
                          </div>
                      ...
                      {meta.topCompetitors.length ? (
                        <div className={`rounded-[22px] border p-3.5 ${rankedGroupTone("amber").panelClass}`}>
                          <div className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${rankedGroupTone("amber").titleClass}`}>
                            高威胁竞品
                          </div>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {meta.topCompetitors.map((value) => (
                              <span
                                key={value}
                                className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs ${rankedGroupTone("amber").pillClass}`}
                              >
                                <span className={`h-1.5 w-1.5 rounded-full ${rankedGroupTone("amber").dotClass}`} />
                                {value}
                              </span>
                            ))}
                          </div>
                      ...
                      {meta.topPartners.length ? (
                        <div className={`rounded-[22px] border p-3.5 ${rankedGroupTone("emerald").panelClass}`}>
                          <div className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${rankedGroupTone("emerald").titleClass}`}>
                            高影响力伙伴
                          </div>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {meta.topPartners.map((value) => (
                              <span
                                key={value}
                                className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs ${rankedGroupTone("emerald").pillClass}`}
                              >
                                <span className={`h-1.5 w-1.5 rounded-full ${rankedGroupTone("emerald").dotClass}`} />
                                {value}
                              </span>
                            ))}
                          </div>
